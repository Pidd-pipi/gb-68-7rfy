...
雨量传感器上报的是雨量计自上次清零以来的**累计毫米数**，系统不会把快照直接相加，而是在每次录入时保存相对上一条读数的增量（`rainfall_increment`）：

- 读数正常增长：增量 = 本次值 − 上一条值；
- 同一累计值重复上报：增量记 0，不再虚增降雨量；
- 读数回落（如雨量计清零/重置）：按重置处理，增量采用新值，旧累计不参与求和；
- 上报时间早于该设备上一条记录、设备不存在或读数为负时拒绝录入（分别返回 400/404）。

按设备查询最近两小时降雨量（各次增量求和）：

```bash
curl -H "Authorization: Bearer <token>" \
  http://localhost:3109/api/sensors/rainfall/{device_id}/last-2h
```

定时灌溉与条件灌溉的避雨判断保持原规则：最近两小时累计降雨 **超过 5 毫米**时跳过灌溉。历史明细与手动灌溉不受影响。
...
| 数据采集 | 土壤湿度、环境温度、降雨量等传感器数据采集，时序存储；雨量按累计读数的**增量**落库，支持按设备查询最近两小时降雨量 |
