...
type SensorData struct {
	...
	// RainfallIncrement 为本次累计雨量读数相对上一条读数的增量（毫米）。
	// 仅雨量数据使用；读数回落（雨量计重置）时采用新值作为增量。
	RainfallIncrement *float64  `json:"rainfall_increment,omitempty" gorm:"type:decimal(10,2)"`
...
// 雨量传感器相关常量
const (
	DataTypeRainfall      = "rainfall"
	RainfallUnit          = "mm"
	RainfallWindow        = 2 * time.Hour // 避雨判断与近两小时降雨量查询窗口
	RainfallSkipThreshold = 5.0           // 窗口内累计降雨达到该毫米数则跳过灌溉
)
